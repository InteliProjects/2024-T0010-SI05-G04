...
export const useRollups = (dAddress: string): RollupsContracts | undefined => {
    ...
    // Efeito para ser executado quando a carteira ou a cadeia conectada mudar
    useEffect(() => {
        // Função para conectar e obter os contratos Rollups
        const connect = async (
            chain: ConnectedChain
        ): Promise<RollupsContracts> => {
            // Instância do provedor Web3
            const provider = new ethers.providers.Web3Provider(
                connectedWallet.provider
            );
            const signer = provider.getSigner();

            // Obtenção dos endereços dos contratos a partir das configurações com base na cadeia conectada
            ...
            // Retorna um objeto contendo todos os contratos Rollups
            return {
                dappContract,
                signer,
                relayContract,
        ...
        // Verifica se a carteira e a cadeia conectada existem
        if (connectedWallet?.provider && connectedChain) {
            // Conecta e obtém os contratos Rollups e, em seguida, atualiza o estado com os contratos obtidos
            connect(connectedChain).then((contracts) => {
                setContracts(contracts);
            });
        }
    }, [connectedWallet, connectedChain, dappAddress]); // Dependências do efeito

    // Retorna os contratos Rollups
    return contracts;
};
